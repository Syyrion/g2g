...
int main(int argc, char **argv)
{
    ...
    if ((gSimpleInfill > 0) && (gFillRadius > eps))
    {
        ...
        if (gScanLineVertical)
        {
            do_vertical_infill(pgn_union, fin_polygons);
        }
        else if (gScanLineHorizontal)
        {
            do_horizontal_infill(pgn_union, fin_polygons);
        }
        else
        { // error
    ...
    else if ((gRadius > eps) || (gFillRadius > eps))
    {
        ...
        if (gInvertFlag)
        {
            ...
            if (gShowComments)
            {
                fprintf(gOutStream, "( inverted selection radius %f, fill radius %f )\n", gRadius, gFillRadius);
            }

            invert(offset_polygons, inverted_polygons);
            if (gScanLineZenGarden)
            {
                do_zen(inverted_polygons, offset_polygons);
            }
            else if (gScanLineVertical)
            {
                do_vertical(inverted_polygons, offset_polygons);
            }
            else if (gScanLineHorizontal)
            {
                do_horizontal(inverted_polygons, offset_polygons);
            }
        }
        else
        {
